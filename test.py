...
def preprocess_micro_doppler(image):
    """Apply noise reduction and normalization for micro-Doppler spectrograms."""
    image = median_filter(image, size=3)  # Reduce noise
    image = (image - np.min(image)) / (np.max(image) - np.min(image) + 1e-6)  # Normalize
    image = image / 255.0  # Scale to [0, 1]
    return image

datagen = ImageDataGenerator(
    preprocessing_function=preprocess_micro_doppler,
    rescale=1./255,
